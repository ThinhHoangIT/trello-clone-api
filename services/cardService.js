...
const addMemberInChecklistItem = async (
  cardId,
  listId,
  boardId,
  user,
  ...
  try {
    ...
    board.activity.unshift({
      user: user._id,
      name: user.name,
      action: `added '${member.name}' to ${checklistItemName} of ${checklistName}`,
      color: user.color,
    });
    board.save();
...
const deleteMemberInChecklistItem = async (
  cardId,
  listId,
  boardId,
  user,
  ...
  try {
    ...
    if (!validate) {
      errMessage: "You dont have permission to delete member from this checklist item";
    }

    //get member
    const tempMember = await userModel.findById(memberId);

    // get checklistName & checklistItemName
    let checklistName = "";
    let checklistItemName = "";

    //Delete member from checklistItem
    card.checklists = card.checklists.map((list) => {
      if (list._id.toString() == checklistId.toString()) {
        checklistName = list.title;
        list.items = list.items.map((item) => {
    ...
    board.activity.unshift({
      ...
      action:
        tempMember.name === user.name
          ? `left ${checklistItemName} of ${checklistName}`
          : `removed '${tempMember.name}' from ${checklistItemName} of ${checklistName}`,
      color: user.color,
    });
    board.save();
...
module.exports = {
  ...
  deleteChecklist,
  addChecklistItem,
  addMemberInChecklistItem,
  deleteMemberInChecklistItem,
